# AITrace Replay Contracts Persistence

## Scope

AITrace Replay Contracts owns ref-only replay bundle and divergence evidence owner. This document is package-local and is the persistence contract for `core/replay_contracts` in `AITrace`.
...
Memory/ref-only replay evidence by default; capture :off is valid.
...
```bash
cd core/replay_contracts && mix test; root verification sequence
```
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. Retained replay refs require explicit capture posture and store refs.
