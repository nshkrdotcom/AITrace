...
AITrace owns trace, export, capture, replay, and redaction boundary owner. This document is package-local and is the persistence contract for `.` in `AITrace`.
...
In-memory collector and explicit exporters. Application env exporters are standalone only; governed export requires explicit exporters.
...
```bash
mix test; mix docs; mix ci when available
```
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. File export receipts or future retained capture only when explicit exporter and release/evidence refs are supplied.
...
No database migrations. File export writes bounded artifacts and adjacent receipts.
