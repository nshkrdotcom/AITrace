# AITrace Replay Engine Persistence

## Scope

AITrace Replay Engine owns deterministic replay execution owner without live provider effects. This document is package-local and is the persistence contract for `core/replay_engine` in `AITrace`.
...
Memory replay execution by default.
...
```bash
cd core/replay_engine && mix test; root verification sequence
```
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. Retained replay inputs must be explicit redacted refs; no raw prompt/provider body rehydration.
